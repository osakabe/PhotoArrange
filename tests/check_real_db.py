...
def check_db():
    ...
    cursor.execute("SELECT group_id, count(*) FROM media WHERE group_id IS NOT NULL GROUP BY group_id HAVING count(*) > 1")
    ...
    cursor.execute("SELECT count(*) FROM media WHERE group_id IS NOT NULL AND group_id NOT IN (SELECT group_id FROM duplicate_groups)")
    ...
    query = f"SELECT count(*) FROM duplicate_groups dg JOIN media m ON dg.group_id = m.group_id WHERE m.is_in_trash = 0 GROUP BY dg.group_id HAVING COUNT(*) > 1"

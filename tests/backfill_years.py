...
def backfill():
    ...
    try:
        ...
        if "year" not in cols or "month" not in cols:
            ...
            if "year" not in cols:
                conn.execute("ALTER TABLE media ADD COLUMN year INTEGER")
            if "month" not in cols:
                conn.execute("ALTER TABLE media ADD COLUMN month INTEGER")
        ...
        cursor = conn.execute(
            "SELECT file_path, capture_date, metadata_json FROM media WHERE (year IS NULL OR year = 0)"
        )
        ...
        for path, cap_date, meta_json in rows:
            ...
            if y and m:
                conn.execute(
                    "UPDATE media SET year = ?, month = ? WHERE file_path = ?", (y, m, path)
                )

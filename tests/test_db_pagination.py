...
def test_pagination():
    ...
    try:
        ...
        with db.get_connection() as conn:
            ...
            for i in range(50):
                cursor.execute(
                    "INSERT INTO faces (file_path, vector_blob, bbox_json, cluster_id, is_ignored) VALUES (?, ?, ?, ?, ?)",
            ...
            cursor.execute(
                "INSERT INTO clusters (cluster_id, custom_name) VALUES (?, ?)", (1, "Person One")
            )
            conn.commit()
        ...
        assert counts["total_persons"] == 80, (
            f"Expected 80 total non-ignored, got {counts['total_persons']}"
        )
        ...
        with db.get_connection() as conn:
            cursor = conn.execute(
                "EXPLAIN QUERY PLAN SELECT face_id FROM faces WHERE cluster_id = ? AND is_ignored = 0 ORDER BY face_id DESC LIMIT 20 OFFSET 0",
                (1,),
            )
            ...
            cursor = conn.execute(
                "EXPLAIN QUERY PLAN SELECT face_id FROM faces WHERE is_ignored = 1 ORDER BY face_id DESC LIMIT 20 OFFSET 0"
            )
            plan = "\n".join([str(row) for row in cursor.fetchall()])

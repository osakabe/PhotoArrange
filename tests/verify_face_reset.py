...
def verify_reset():
    ...
    with db.get_connection() as conn:
        # Add some media
        conn.execute("INSERT INTO media (file_path, last_modified, is_in_trash) VALUES (?, ?, ?)", 
    ...
    with db.get_connection() as conn:
        ...
        conn.execute("INSERT INTO faces (file_path, cluster_id, vector_blob, bbox_json) VALUES (?, ?, ?, ?)", 
                     ("c:/photo1.jpg", 1, b'\x00' * 2048, '[]'))
        
        # Add some ignored vectors
        conn.execute("INSERT INTO ignored_person_vectors (id, vector_blob) VALUES (?, ?)", (1, b'\x00' * 2048))
        conn.commit()
    ...
    with db.get_connection() as conn:
        ...
        print(f"Before reset: Faces={face_count}, Clusters={cluster_count}, Ignored={ignored_count}")
        assert face_count == 1
    ...
    with db.get_connection() as conn:
        ...
        cursor = conn.execute("SELECT COUNT(*) FROM sqlite_sequence WHERE name IN ('faces', 'clusters', 'ignored_person_vectors')")
    ...
    with db.get_connection() as conn:
        cid = conn.execute("SELECT cluster_id FROM clusters WHERE custom_name='New Person'").fetchone()[0]

...
class TestPersonLogic(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with cls.db.get_connection() as conn:
            ...
            conn.execute("INSERT INTO faces (file_path, vector_blob, bbox_json, cluster_id) VALUES ('test.jpg', ?, '[0,0,10,10]', NULL)", (dummy_vec,))
        cls.face_id = 1
    ...
    def test_01_register_new(self):
        ...
        def on_finished(success, msg):
            ...
            if not success: print(f"Worker Task Failed: {msg}")
        ...
        with self.db.get_connection() as conn:
            row = conn.execute("SELECT cluster_id, is_ignored FROM faces WHERE face_id = ?", (self.face_id,)).fetchone()
    ...
    def test_02_associate_existing(self):
        ...
        with self.db.get_connection() as conn:
            row = conn.execute("SELECT cluster_id FROM faces WHERE face_id = ?", (self.face_id,)).fetchone()
    ...
    def test_03_ignore_face(self):
        ...
        with self.db.get_connection() as conn:
            row = conn.execute("SELECT is_ignored FROM faces WHERE face_id = ?", (self.face_id,)).fetchone()
    ...
    def test_04_ignore_cluster(self):
        ...
        with self.db.get_connection() as conn:
            import numpy as np
            dummy_vec = np.zeros(512, dtype='float32').tobytes()
            conn.execute("INSERT INTO faces (file_path, vector_blob, cluster_id) VALUES ('test2.jpg', ?, ?)", (dummy_vec, cid))
        ...
        with self.db.get_connection() as conn:
            row = conn.execute("SELECT is_ignored FROM clusters WHERE cluster_id = ?", (cid,)).fetchone()
            self.assertEqual(row[0], 1)
            # Verify all faces in cluster are ignored
            rows = conn.execute("SELECT is_ignored FROM faces WHERE cluster_id = ?", (cid,)).fetchall()
            for r in rows:

...
def run_formal_audit():
    ...
    for _ in range(100):  # 10 seconds max
        ...
        if view.face_model.rowCount() > 0:
            ...
            if view.face_model.rowCount() >= 200:
                break
        time.sleep(0.1)
    ...
    if selected_ids:
        ...
        logger.info(
            f"Selected {len(selected_ids)} faces. Current ignored in DB: {ignore_count_before}"
        )
        ...
        if ignore_count_after == ignore_count_before + len(selected_ids):
            ...
            results.append(
                (
                    "Data Integrity (Bulk Ignore)",
                    f"FAIL: Expected {ignore_count_before + len(selected_ids)}, got {ignore_count_after}",
                )
            )
    else:

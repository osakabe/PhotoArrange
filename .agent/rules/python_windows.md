...
- **有効化**: `conda activate photo_env` (プロジェクト標準環境名)
- **実行**: Python スクリプトの実行時は、必ず `conda activate photo_env` を行うか、仮想環境内の `python.exe` を直接指定して実行してください。
- **理由**: OS から独立したクリーンな実行環境でライブラリの依存関係を正しく解決し、グローバル環境との衝突を防ぐためです。
...
# Python Development Rules & Lessons Learned (PhotoArrange 標準)

## 4. コーディング標準
- **Ruff**: コードのリンティングおよびフォーマットに必ず **ruff** を使用すること。提出前に `ruff check` および `ruff format` をパスさせること。
- **Type Hints**: 明示的な型ヒント (typing) を可能な限り使用すること。
- **PEP 8**: 最新の Python イディオムと PEP 8 に準拠すること。

## 5. Windows Path Normalization Rules (Strict)
...
- **DLL Isolation**: PyTorch と InsightFace の DLL (libiomp5md.dll) 競合を避けるため、起動時に環境変数を適切に設定すること。

## 8. SQLite Concurrency & Grouping (WAL Mode)
- **ルール**: `PRAGMA journal_mode=WAL;` および `PRAGMA busy_timeout=5000;` を使用し、読込をブロックさせないようにします。

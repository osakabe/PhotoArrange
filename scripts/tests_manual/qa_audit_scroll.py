...
class AuditWindow(QMainWindow):
    ...
    def finalize_audit(self):
        ...
        logger.info(
            f"Heights: 1000px width -> {initial_height}px, 600px width -> {h_600}px, 1200px width -> {h_1200}px"
        )
        ...
        if h_600 > initial_height and h_1200 < initial_height:
            ...
            logger.warning(
                "RESULT: Layout stability may have ISSUES (Height didn't adapt as expected)."
            )

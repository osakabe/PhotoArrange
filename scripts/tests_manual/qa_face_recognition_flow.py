...
from main import FaceRecognitionWorker
...
def test_flow():
    ...
    db.add_media_batch(
        [(img_path, 0, "{}", None, 0, 0, 0, None, None, None, 0, 0, None, 0, 0, "", None, None)]
    )
    ...
    worker.finished_all = type(
        "Signal", (), {"emit": lambda ok, msg: logger.info(f"Finished: {ok}, {msg}")}
    )()

...
MODEL_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "insightface", "models", "buffalo_l")
)
...
def setup_test_env():
    ...
    # ダミー画像の作成
    for i in range(5):
        Image.new("RGB", (640, 640), color=(i * 50, i * 50, i * 50)).save(
            os.path.join(TEST_IMG_DIR, f"dummy_{i}.jpg")
        )
...
class MockSignal:
    ...
    def emit(self, *args):
        logger.info(f"Signal [{self.name}]: {args}")
...
def run_tests():
    ...
    logger.info(
        f"VRAM: {gpu_mem_before:.1f}MB -> {gpu_mem_after:.1f}MB (torch.cuda.empty_cache()によりベースラインに戻ることを確認)"
    )

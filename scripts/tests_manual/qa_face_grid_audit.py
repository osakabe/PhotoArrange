...
from ui.widgets.face_manager_view import FaceItem, FaceManagerView
...
class MockDB:
    def __init__(self):
        self.faces = [
            {"face_id": i, "file_path": "dummy.jpg", "bbox": [0, 0, 100, 100], "person_id": i}
            for i in range(20)
        ]
...
def run_audit():
    ...
    if 135 <= size.width() <= 145 and 155 <= size.height() <= 165:
        ...
        results.append(
            ("Item Size (Target 140x160)", f"FAIL: Actual {size.width()}x{size.height()}")
        )
    ...
    logger.info(
        f"Grid Width: {grid_w}, Expected Columns: {expected_cols}, Actual Items: {layout.count()}"
    )
    ...
    if actual_height >= expected_height - 10:  # Allow some margin
        ...
        results.append(
            (
                ...
                f"FAIL: Height too small ({actual_height} < {expected_height})",
            )
        )
    ...
    if new_height > actual_height:
        ...
        results.append(
            ("Dynamic Resizing (Cols -> Rows)", "FAIL: Height did not increase on narrowing window")
        )

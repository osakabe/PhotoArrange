...
class FaceVerificationDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.title_label.setStyleSheet(
            "font-size: 18px; font-weight: bold; color: #3D5AFE; margin-bottom: 10px;"
        )
    ...
    def create_person_frame(self, title):
        ...
        img_label.setStyleSheet(
            "background-color: #0F111A; border: 2px solid #2D324A; border-radius: 10px;"
        )
    ...
    def on_face_loaded(self, frame_idx, pix, name):
        ...
        if pix and not pix.isNull():
            target_frame.img_label.setPixmap(
    ...
    def on_merge(self):
        ...
        if f"Person {c1 + 1}" != name_a:
            target_name = name_a
        elif f"Person {c2 + 1}" != name_b:
            target_name = name_b
